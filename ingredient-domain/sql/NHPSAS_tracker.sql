--------------------------------------------------------------------------------
-- The NHPSAS_tracker.sql contains the scripts required to build the tables
-- that track ingredient database changes for nhpsas.  This is a run once
-- script.  The build_script.sql file has been modified to not delete
-- the tables created by this script.
--------------------------------------------------------------------------------

-- Clean Up Lines (Drop all related objects to ensure a clean load)
-- Note: Don't do this if you are interested in preserving history.
-- DROP TABLE Error_Logs;
-- DROP TABLE Log_Details;
-- DROP TABLE Logs;
-- DROP TABLE NHPSAS_INGREDIENT_SYNC;
