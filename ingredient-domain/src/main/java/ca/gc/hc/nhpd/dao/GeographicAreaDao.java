...
import ca.gc.hc.nhpd.model.GeographicArea;
...
/*******************************************************************************
 * An object used to retrieve instances of GeographicArea from persistent store.
 * @see ca.gc.hc.nhpd.model.GeographicArea
 */
public class GeographicAreaDao extends AbstractDao {
     ...
     */
    private final static String TABLE_NAME="geographic_areas";
    private final static String ID_COLUMN_NAME="geogarea_id";
    private final static String FRENCH_COLUMN_NAME="geogarea_name_fr";
    private final static String ENGLISH_COLUMN_NAME="geogarea_name_eng";
    ...
    /***************************************************************************
     * Gets a collection of all the GeographicAreas in the persistent store.
     * Note that these are sorted by their English names.
     * @return all instances of the GeographicAreas that are in the persistent
     *         store.
    ...
    /***************************************************************************
     * Gets the GeographicArea with the passed ID from the persistent store.
     * Returns null if none is found.
     * @param id the unique key of the GeographicArea in the persistent store.
     * @param lock true if the corresponding record should automatically be
     *        locked from updates in the persistent store.
     * @return the GeographicArea, if any, with the passed ID in the persistent
     *         store.
     */
    ...
    /***************************************************************************
     * Gets the GeographicArea with the passed constraints from the persistent
    ...
    public PagedList findForFilter(String name, String language) {
        ...
        queryText.append("SELECT DISTINCT ga.* ");
        queryText.append("FROM geographic_areas ga ");

        // Search units accross the Code, French and English names.
        nameExpression = nameExpression.toUpperCase();
        queryText
                .append("WHERE (UPPER(ga.geogarea_name_eng)" + nameExpression);
        queryText.append("  OR UPPER(ga.geogarea_name_fr)" + nameExpression);

        queryText.append("  ) ");
        queryText.append("  ORDER BY UPPER(ga.geogarea_name_eng)");
