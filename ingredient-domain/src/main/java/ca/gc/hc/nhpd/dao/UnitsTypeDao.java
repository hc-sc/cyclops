 ...
 */
public class UnitsTypeDao extends AbstractDao {	

	private static final Logger log = Logger.getLogger(UnitsTypeDao.class);
     ...
     */
    private final static String TABLE_NAME="units_types";
    private final static String ID_COLUMN_NAME="unitstype_id";
    private final static String FRENCH_COLUMN_NAME="unitstype_name_fr";
    private final static String ENGLISH_COLUMN_NAME="unitstype_name_eng";
	...
	/***************************************************************************
		* Gets the UnitsType with the passed constraints from the persistent
		...
		*        This looks for all matches in the unitstype's name, as well as in 
		...
		* @return the UnitsType in the persistent store that match the passed
		*         constraints.
		*/
	...
	public PagedList findForFilter(String name, String language) {
		...
		queryText.append("SELECT DISTINCT ut.* ");
		queryText.append("FROM units_types ut ");

		// Search units accross the Code, French and English names.
		nameExpression = nameExpression.toUpperCase();
		queryText.append("WHERE (UPPER(ut.unitstype_name_eng)" + nameExpression);
		queryText.append("  OR UPPER(ut.unitstype_name_fr)" + nameExpression);        
		queryText.append("  ) ");
		if (Locale.FRENCH.getLanguage().equals(language)) {
			queryText.append("  ORDER BY UPPER(ut.unitstype_name_fr)");
		} else {
			queryText.append("  ORDER BY UPPER(ut.unitstype_name_eng)");
		}
