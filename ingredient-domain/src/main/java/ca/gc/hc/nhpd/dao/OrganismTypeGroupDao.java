 ...
 */
@Component
public class OrganismTypeGroupDao extends AbstractDao {
     ...
     */
    private final static String TABLE_NAME="organism_type_groups";
    private final static String ID_COLUMN_NAME="orgtypegrp_id";
    private final static String FRENCH_COLUMN_NAME="orgtypegrp_name_fr";
    private final static String ENGLISH_COLUMN_NAME="orgtypegrp_name_eng";
    
    /***************************************************************************
     * Generic Constructor.
     */
    public OrganismTypeGroupDao() {
        super(OrganismTypeGroup.class);
    }
    ...
    /***************************************************************************
     * Gets a collection of all the OrganismTypes in the persistent store.
     * Note that these are sorted by their English names.
     * @return all instances of the OrganismTypes that are in the persistent store.
    ...
    /***************************************************************************
     * Gets a collection of all the Organism Type Group in the persistent
     * store in the language specified.  Note that the sorting is based on the
     * language that is passed.
     * 
     * @return all instances of the Organism Type Group that are in the
     *         persistent store.
     * @throws HibernateException if a problem is encountered.
     */
    public List<OrganismTypeGroup> findAll(String language) throws HibernateException {
        List order = new ArrayList();
     ...
     * @param id the unique key of the OrganismType in the persistent store.
     * @param lock true if the corresponding record should automatically be
     *        locked from updates in the persistent store.
     * @return the OrganismType, if any, with the passed ID in the persistent store.
     */
    ...
    /***************************************************************************
     * Gets the OrganismTypeGroup with the passed constraints from the persistent
    ...
    public PagedList findForFilter(String name, String language) {
        ...
        queryText.append("SELECT DISTINCT otg.* ");
        queryText.append("FROM organism_type_groups otg ");

        // Search units accross the Code, French and English names.
        nameExpression = nameExpression.toUpperCase();
        queryText
                .append("WHERE (UPPER(otg.orgtypegrp_name_eng)" + nameExpression);
        queryText.append("  OR UPPER(otg.orgtypegrp_name_fr)" + nameExpression);

        queryText.append("  ) ");
        queryText.append("  ORDER BY UPPER(otg.orgtypegrp_name_eng)");
