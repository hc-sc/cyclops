...
import ca.gc.hc.nhpd.model.UserRole;
...
/*******************************************************************************
 * An object used to retrieve instances of User Role from persistent store.
 * @see ca.gc.hc.nhpd.model.UserRole
 */
public class UserRoleDao extends AbstractDao {
     ...
     */
    private final static String TABLE_NAME="user_roles";
    private final static String ID_COLUMN_NAME="userrole_id";
    private final static String FRENCH_COLUMN_NAME="userrole_name_fr";
    private final static String ENGLISH_COLUMN_NAME="userrole_name_eng";
    ...
    /***************************************************************************
     * Gets a collection of all the User Roles in the persistent store.
     * Note that these are sorted by their codes.
     * @return all instances of the User Roles that are in the persistent store.
     * @throws HibernateException if a problem is encountered.
     */
    public List findAll() throws HibernateException {
        List order = new ArrayList();

        order.add(Order.asc("roleNameE"));
        return findByCriteria(null, order);
    }

    /***************************************************************************
     * Gets the User Roles with the passed ID from the persistent store.
     * Returns null if none is found.
     * @param id the unique key of the User Roles in the persistent store.
     * @param lock true if the corresponding record should automatically be
     *        locked from updates in the persistent store.
     * @return the User Role, if any, with the passed ID in the persistent store.
     */
	...
	/***************************************************************************
		* Gets the User Role with the passed constraints from the persistent
	...
	public PagedList findForFilter(String name, String language) {
		...
		queryText.append("FROM user_roles u ");

		// Search units accross the Code, French and English names.
		nameExpression = nameExpression.toUpperCase();
		queryText.append("WHERE (UPPER(u.userrole_name_eng)" + nameExpression);
		queryText.append("  OR UPPER(u.userrole_name_fr)" + nameExpression);
		
		queryText.append("  ) ");
		queryText.append("  ORDER BY UPPER(u.userrole_name_eng)");
