...
/*******************************************************************************
 * An object used to retrieve instances of RiskType from persistent store.
 * @see ca.gc.hc.nhpd.model.RiskType
 */
public class RiskTypeDao extends AbstractDao {	

	private static final Logger log = Logger.getLogger(RiskTypeDao.class);
    ...
    /***************************************************************************
     * Gets a collection of all the RiskType in the persistent store.
     * Note that these are sorted by their codes.
     * @return all instances of the RiskType that are in the persistent store.
    ...
    /***************************************************************************
     * Gets the RiskType with the passed ID from the persistent store.
     * Returns null if none is found.
     * @param id the unique key of the RiskType in the persistent store.
     * @param lock true if the corresponding record should automatically be
     *        locked from updates in the persistent store.
     * @return the RiskType, if any, with the passed ID in the persistent store.
     */
	...
	/***************************************************************************
		* Gets the RiskType with the passed constraints from the persistent
		...
		*        This looks for all matches in the RiskType's name, as well as in 
		...
		* @return the RiskType in the persistent store that match the passed
		*         constraints.
		*/
	...
	public PagedList findForFilter(String name, String language) {
		...
		queryText.append("SELECT DISTINCT rt.* ");
		queryText.append("FROM risk_types rt ");

		// Search indication types accross the Code, French and English names.
		nameExpression = nameExpression.toUpperCase();
		queryText.append("WHERE (UPPER(rt.risktype_name_eng)" + nameExpression);
		queryText.append("  OR UPPER(rt.risktype_name_fr)" + nameExpression);        
		queryText.append("  ) ");
		if (Locale.FRENCH.getLanguage().equals(language)) {
			queryText.append("  ORDER BY UPPER(rt.risktype_name_fr)");
		} else {
			queryText.append("  ORDER BY UPPER(rt.risktype_name_eng)");
		}
