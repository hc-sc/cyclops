...
import ca.gc.hc.nhpd.model.MicrobiologicalImpurity;
 ...
 * @see MicrobiologicalImpurity
 */
public class MicrobiologicalImpurityDao extends AbstractDao {
     ...
     * Generic Constructor.
     */
    public MicrobiologicalImpurityDao() {
        super(MicrobiologicalImpurity.class);
    }
    ...
    public Object findById(Long id, boolean lock) {
        ...
        return (MicrobiologicalImpurity)super.findByIdBase(id, lock);
    	} catch ( Exception e ) {
    		e.printStackTrace();
    	}
     ...
     */
     ...
     public PagedList findForFilter(String name, String language) {
         ...
         queryText.append("SELECT DISTINCT bi.* ");
         queryText.append("FROM biological_impurities bi ");

         // Search units accross the Code, French and English names.
         nameExpression = nameExpression.toUpperCase();
         queryText.append("WHERE (UPPER(ir.ingredrole_class_name)" + nameExpression);
         queryText.append("  OR UPPER(ir.ingredrole_class_name)" + nameExpression);
         
         queryText.append("  ) ");
         if (Locale.FRENCH.getLanguage().equals(language)) {
             queryText.append("  ORDER BY UPPER(ir.ingredrole_class_name)");
         } else {
             queryText.append("  ORDER BY UPPER(ir.ingredrole_class_name)");
         }
