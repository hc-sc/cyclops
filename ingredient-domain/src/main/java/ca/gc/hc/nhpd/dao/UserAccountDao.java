...
/*******************************************************************************
 * An object used to retrieve instances of UserAccount from persistent store.
 * @see ca.gc.hc.nhpd.model.UserAccount
 */
public class UserAccountDao extends AbstractDao {
	
	private static final Logger log = Logger.getLogger(UserAccountDao.class);
     ...
     */
    private final static String TABLE_NAME="user_accounts";
    private final static String ID_COLUMN_NAME="useracc_id";
    private final static String FRENCH_COLUMN_NAME="useracc_account_name";
    private final static String ENGLISH_COLUMN_NAME="useracc_account_name";
	...
	public PagedList findForFilter(String name, String language) {
		...
		queryText.append("FROM user_accounts u ");

		// Search units accross the Code, French and English names.
		nameExpression = nameExpression.toUpperCase();
		queryText.append("WHERE (UPPER(u.useracc_account_name)" + nameExpression);
		queryText.append("  OR UPPER(u.useracc_first_name)" + nameExpression);
		queryText.append("  OR UPPER(u.useracc_last_name)" + nameExpression);
        
		queryText.append("  ) ");
		queryText.append("  ORDER BY UPPER(u.useracc_last_name)");
