...
/*******************************************************************************
 * This object represents a preferred term (PT) from the Medical Dictionary for
 * Regulatory Activities (MedDRA) system. It is a distinct descriptor (single
 * medical concept) for a symptom, sign, disease, diagnosis, therapeutic
 * indication, investigation, surgical, or medical procedure, and medical, social,
 * or family history characteristic.
 * @version 1.0
 */
public class MeddraPreferredTerm extends PersistentObject implements Comparable {
	
	private String code;
	private String nameE;
	private String nameF;
	private Set<MeddraHighLevelTerm> highLevelTerms;
	private Set<MeddraLowestLevelTerm> lowestLevelTerms;
    ...
    /***************************************************************************
     */
	public Set<MeddraLowestLevelTerm>  getLowestLevelTerms(){
	 ...
	 * @param newVal
	 */
	public void setLowestLevelTerms(Set<MeddraLowestLevelTerm>  newVal){
		lowestLevelTerms = newVal;
	}
    ...
    public String getValuesAsString(){
        ...
        MeddraLowestLevelTerm lowestLevelTerm;
        ...
        buffer.append(", lowestLevelTerms: ");
        if (getLowestLevelTerms() != null 
            && !getLowestLevelTerms().isEmpty()) {
            first = true;
            buffer.append("[");
            for(Iterator it=getLowestLevelTerms().iterator(); it.hasNext();) {
                if (first) {
                    first = false;
                } else {
                    buffer.append(", ");
                }
                lowestLevelTerm = (MeddraLowestLevelTerm)it.next();
                buffer.append(lowestLevelTerm.getName());
                buffer.append("(" + lowestLevelTerm.getId() + ")");
            }
            buffer.append("]");
        } else {
            buffer.append("none");
