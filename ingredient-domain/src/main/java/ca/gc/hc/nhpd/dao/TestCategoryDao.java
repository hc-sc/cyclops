...
import ca.gc.hc.nhpd.model.TestCategory;
 ...
 */
public class TestCategoryDao extends AbstractDao {
	
	private static final Logger log = Logger.getLogger(TestCategoryDao.class);
     ...
     */
    private final static String TABLE_NAME="test_categories";
    private final static String ID_COLUMN_NAME="testcat_id";
    private final static String FRENCH_COLUMN_NAME="testcat_name_fr";
    private final static String ENGLISH_COLUMN_NAME="testcat_name_eng";
    ...
    public Object findById(Long id, boolean lock) {
        ...
        return (TestCategory)super.findByIdBase(id, lock);
    	} catch ( Exception e ) {
    		e.printStackTrace();
    	}
	...
	*/
	...
	public PagedList findForFilter(String name, String language) {
		...
		queryText.append("SELECT DISTINCT tc.* ");
		queryText.append("FROM test_categories tc ");

		// Search units accross the Code, French and English names.
		nameExpression = nameExpression.toUpperCase();
		queryText.append("WHERE (UPPER(tc.testcat_code)" + nameExpression);
		queryText.append("  OR UPPER(tc.testcat_name_eng)" + nameExpression);
		queryText.append("  OR UPPER(tc.testcat_name_fr)" + nameExpression);
        
		queryText.append("  ) ");
		if (Locale.FRENCH.getLanguage().equals(language)) {
			queryText.append("  ORDER BY UPPER(tc.testcat_code)");
		} else {
			queryText.append("  ORDER BY UPPER(tc.testcat_code)");
		}
