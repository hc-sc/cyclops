...
import ca.gc.hc.nhpd.model.IndicationType;
...
/*******************************************************************************
 * An object used to retrieve instances of IndicationType from persistent store.
 * @see ca.gc.hc.nhpd.model.IndicationType
 */
public class IndicationTypeDao extends AbstractDao {	

	private static final Logger log = Logger.getLogger(IndicationTypeDao.class);
     ...
     */
    private final static String TABLE_NAME="indication_types";
    private final static String ID_COLUMN_NAME="indictype_id";
    private final static String FRENCH_COLUMN_NAME="indictype_name_fr";
    private final static String ENGLISH_COLUMN_NAME="indictype_name_eng";
    private final static String DISCRIMINATOR=null;
    
    /***************************************************************************
     * Generic Constructor.
     */
    public IndicationTypeDao() {
        super(IndicationType.class);
    }
    ...
    /***************************************************************************
     * Gets a collection of all the IndicationType in the persistent store.
     * Note that these are sorted by their codes.
     * @return all instances of the IndicationType that are in the persistent store.
    ...
    /***************************************************************************
     * Gets the IndicationType with the passed ID from the persistent store.
     * Returns null if none is found.
     * @param id the unique key of the IndicationType in the persistent store.
     * @param lock true if the corresponding record should automatically be
     *        locked from updates in the persistent store.
     * @return the IndicationType, if any, with the passed ID in the persistent store.
     */
	...
	/***************************************************************************
		* Gets the IndicationType with the passed constraints from the persistent
		...
		*        This looks for all matches in the IndicationType's name, as well as in 
		...
		* @return the IndicationType in the persistent store that match the passed
		*         constraints.
		*/
	...
	public PagedList findForFilter(String name, String language) {
		...
		queryText.append("SELECT DISTINCT it.* ");
		queryText.append("FROM indication_types it ");

		// Search indication types across the Code, French and English names.
		nameExpression = nameExpression.toUpperCase();
		queryText.append("WHERE (UPPER(it.indictype_name_eng)" + nameExpression);
		queryText.append("  OR UPPER(it.indictype_name_fr)" + nameExpression);        
		queryText.append("  ) ");
		if (Locale.FRENCH.getLanguage().equals(language)) {
			queryText.append("  ORDER BY UPPER(it.indictype_name_fr)");
		} else {
			queryText.append("  ORDER BY UPPER(it.indictype_name_eng)");
		}
