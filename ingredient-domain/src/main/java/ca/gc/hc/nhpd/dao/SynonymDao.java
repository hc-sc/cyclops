...
public class SynonymDao extends AbstractDao {
    ...
    public Object findById(Long id, boolean lock) {
        ...
        return (Synonym)super.findByIdBase(id, lock);
    	} catch ( Exception e ) {
    		e.printStackTrace();
    	}
    ...
    public StringBuffer getSearchResultSQLStatement(boolean isFrench,
        ...
        queryText.append("s.synonym_id, ");
        
        if (isFrench) {
            queryText.append("s.synonym_name_fr ");
        } else {
            queryText.append("s.synonym_name_eng ");
        }
        
        queryText.append("FROM ");
        queryText.append("synonyms s ");

        // NB. Search terms are language specific
        if (searchString != null && !searchString.equals(EMPTY_STRING)) {
            queryText.append("WHERE ");
            if (isFrench) {
                queryText.append("LOWER(s.synonym_name_fr) ");
            } else {
                queryText.append("LOWER(s.synonym_name_eng) ");
            }
        ...
        if (isFrench) {
            queryText.append("ORDER BY LOWER(s.synonym_name_fr) ");
        } else {
            queryText.append("ORDER BY LOWER(s.synonym_name_eng) ");
        }
