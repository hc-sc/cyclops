...
import ca.gc.hc.nhpd.model.TestMethod;
 ...
 */
public class TestMethodDao extends AbstractDao {
	
	private static final Logger log = Logger.getLogger(TestMethodDao.class);
     ...
     */
    private final static String TABLE_NAME="test_methods";
    private final static String ID_COLUMN_NAME="testmethod_id";
    private final static String FRENCH_COLUMN_NAME="testmethod_name_fr";
    private final static String ENGLISH_COLUMN_NAME="testmethod_name_eng";
    ...
    public Object findById(Long id, boolean lock) {
        ...
        return (TestMethod)super.findByIdBase(id, lock);
    	} catch ( Exception e ) {
    		e.printStackTrace();
    	}
	...
	public PagedList findForFilter(String name, String language) {
		...
		queryText.append("SELECT DISTINCT tm.* ");
		queryText.append("FROM test_methods tm ");

		// Search units accross the Code, French and English names.
		nameExpression = nameExpression.toUpperCase();
		queryText.append("WHERE (UPPER(tm.testmethod_code)" + nameExpression);
		queryText.append("  OR UPPER(tm.testmethod_name_eng)" + nameExpression);
		queryText.append("  OR UPPER(tm.testmethod_name_fr)" + nameExpression);
        
		queryText.append("  ) ");
		if (Locale.FRENCH.getLanguage().equals(language)) {
			queryText.append("  ORDER BY UPPER(tm.testmethod_code)");
		} else {
			queryText.append("  ORDER BY UPPER(tm.testmethod_code)");
		}
